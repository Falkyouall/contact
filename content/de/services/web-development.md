---
title: Webentwicklung
subtitle: Software, die zu deinem Business passt, nicht umgekehrt
date: 2026-02-21
description: "Maßgeschneiderte Webanwendungen, die ein echtes Problem lösen: Konfiguratoren, Dashboards, interne Tools, Kundenportale."
contactHeading: "Lass uns deine Webanwendung bauen"
---

Die meisten "Webentwicklungs"-Pitches sind im Grunde "Ich kann React". Das hier ist anders. Ich baue die Art von [Anwendungen, hinter denen echte Komplexität steckt](/de/services/fullstack-development-ts): Konfiguratoren, Dashboards, interne Tools, Kundenportale; Apps, die in kein fertiges Produkt von der Stange passen.

## Kennst du das?

- Ein SaaS-Tool deckt fast alles ab, nur die eine Sache, die du wirklich brauchst, eben nicht
- Dein Team hat etwas in Excel zusammengeschraubt, das inzwischen mehr Schmerzen verursacht als es löst
- Die Oberfläche, die du ausliefern willst, muss echte Komplexität bewältigen (Formulare, Flows, Berechtigungen), ohne Nutzer in den Wahnsinn zu treiben

## Was sich ändert

- Eine App, die dein echtes Problem löst, nicht ein generisches
- Code, den der nächste Entwickler ohne Schatzkarte versteht
- Performance ist kein nachträglicher Gedanke: schnelle Ladezeiten, reale Geräte, Barrierefreiheit von Anfang an

## Wie ich arbeite

Ich starte meistens beim Workflow, nicht bei der Technologie. Wir schauen uns gemeinsam an, wer was wo benutzt und wo es weh tut, und bauen dann [das kleinste Stück, das die Kern-Hypothese beweist](/de/services/rapid-prototyping). Von dort iterieren wir. Kein 6-Monate-"Phase 1", die nichts ausliefert.

Ich arbeite [KI-gestützt, aber Mensch-geführt](/de/services/ai-integration). Die Entscheidungen, die zählen (Architektur, Edge Cases, was *nicht* gebaut wird), die treffe ich.

## Aus der Praxis

Zum Beispiel: eine barrierefreie, WCAG-konforme Landingpage für ein Finanzprodukt, gebaut mit React und TypeScript, deployt auf Azure Container Apps.

## Wie Preise entstehen

Keine Stundensätze, keine Preisliste. Am Anfang steht ein Gespräch über das Problem, und darüber, was seine Lösung für dein Geschäft wert ist. Danach bekommst du ein Festpreis-Angebot in drei Zuschnitten (schlank, komplett, premium), damit du Umfang und Budget selbst austarieren kannst.

---

*Tools, zu denen ich gerne greife: React, TypeScript, TanStack Start/Router, Node.js, Vite, Docker, GitHub Actions. Der Stack passt sich dem Projekt an, nicht umgekehrt.*

*Verwandte Leistung: [CMS-Websites](/de/services/cms-websites)*
